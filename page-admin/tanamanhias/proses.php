...
if(isset($_POST['add'])) {
	...
	if(mysqli_num_rows($sql_cek_identitas) > 0) {
		...
		if ($gambar != '') {
			...
			mysqli_query($con, "INSERT INTO tb_tanamanhias (id_tanamanhias, nama_pohon, nama_latin, deskripsi, manfaat, tahun_tanam, jumlah, luas, gambar) VALUES ('$uuid', '$nama_pohon', '$nama_latin', '$deskripsi', '$manfaat', '$tahun_tanam', '$jumlah', '$luas', '$gambar')") or die (mysqli_error($con));
	...
	$tmp_gambar = $_FILES['gambar']['tmp_name'];
		...
		if ($gambar != '') {
			if (move_uploaded_file($tmp_gambar, "../../_assets/img3/{$gambar}")) {
				mysqli_query($con, "UPDATE tb_tanamanhias SET gambar='$gambar' WHERE id_tanamanhias = '$_POST[id_tanamanhias]'") or die (mysqli_error($con));
				echo "<script>	
				window.location='data.php';
				</script>";
			}
	...
	mysqli_query($con, "UPDATE tb_tanamanhias SET nama_pohon = '$nama_pohon', nama_latin = '$nama_latin', deskripsi = '$deskripsi', manfaat = '$manfaat', tahun_tanam = '$tahun_tanam', jumlah = '$jumlah', luas = '$luas' WHERE id_tanamanhias = '$id'") or die (mysqli_error($con));
	echo "<script>window.location='data.php';</script>";
?>